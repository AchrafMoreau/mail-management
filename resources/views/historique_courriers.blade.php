...
@section('content')
    <style>
        ...
        .search-form .search-icon {
            ...
            margin-left: 10px;
            margin-top: 30px; /* Ajoutez cette ligne pour descendre l'icône un peu plus bas */
        }
    ...
    <div class="container">
        <h1 class="page-title">Historique des Courriers</h1>
        <form class="search-form" action="{{ route('historique_courriers') }}" method="GET">
            ...
            <button type="submit" class="search-icon" aria-label="Rechercher">
                <!-- Icône de recherche en image -->
            </button>
        </form>

        @if ($courriers->isEmpty())
            <p class="no-results">Aucun courrier trouvé pour les critères sélectionnés.</p>
        @else
            <table>
                <thead>
                    <tr>
                ...
                <tbody>
                    @foreach ($courriers as $courrier)
                        <tr>
                            ...
                            <td class="date">{{ $courrier->type }}</td>
                            <td>{{ $courrier->date }}</td>
                            ...
                            <td>
                                @if ($courrier->image_scan)
                                    <span class="icon" onclick="openModal('{{ asset('storage/uploads/courriers/' . $courrier->image_scan) }}')">
